describe('page bar', function () {
  ...
  it('handles pages which have previously been seen correctly', function (done) {
    _initWithPage('https://pcmag.com/article2/0,2817,2361879,00.asp')
    pgzp.url_list.push('https://pcmag.com/article2/0,2817,2361878,00.asp')
    readInDom('inputs/page-bar-3.html', function (body) {
      var allNextLinks = pgzp.getAllNextLinks(body)
      var results = pgzp.getCurrentPageNumberFromPageBar(allNextLinks)
      expect(results[1].text).toBe('5')
      expect(results[2]).toBe(120)
      expect(pgzp.getNextLink(body).url).toBe(
        'https://pcmag.com/article2/0,2817,2361880,00.asp'
      )
      done()
    })
  })
...
describe('getAllNextLinks', function () {
  it('loads correct urls', function () {
    ...
    _initWithPage('https://test.com/test.hml')
    var html = "<a href='https://test.com/test2.hml'>adsf</a>"
    assertLinks(html, 1)
    ...
    html = "<a href='https://test2.com/test2.hml'>adsf</a>"
    assertLinks(html, 0)

    //not anchor link to current page
    html = "<a href='https://test.com/test.hml#anchor'>adsf</a>"
    assertLinks(html, 0)
  })
  ...
  it('gets all parts of the link', function (done) {
    ...
    readInDom('inputs/all-urls-1.html', function (body) {
      ...
      expect(nextLinks[0].url).toBe('https://test.com/test2.hml')
      expect(nextLinks[1].url).toBe('https://test.com/test2.hml')
      expect(nextLinks[2].url).toBe('https://test.com/test2.hml')
      expect(nextLinks[3].url).toBe('https://test.com/test2.hml')
      expect(nextLinks[4].url).toBe('https://test.com/test2.hml')
      done()
    })
  })
})

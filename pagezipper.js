...
function PageZipper() {
	this.nextSynonyms = [{ syn: "next", weight: 100 }, { syn: "older", weight: 80 }, { syn: "previous", weight: 60 }, { syn: "forward", weight: 50 }, { syn: "continue", weight: 45 }, { syn: ">", weight: 40, humanReadableOnly: true }, { syn: ">>", weight: 30, humanReadableOnly: true }, { syn: "more", weight: 20 }, { syn: "page", weight: 10 }, { syn: "part", weight: 5 }, { syn: "-1", weight: 0, humanReadableOnly: true, pageBar: true }];
...
PageZipper.prototype.mainBlock = function () {
	...
	if (!pgzp.is_loading_page && pgzp.pages[pgzp.pages.length - 1]["nextLinkObj"] && pgzp.pages.length - currPageIndex - 1 < pgzp.minimumPageBuffer) {
	...
	pgzp.depthFirstRecursion(pageHtml, function (curr) {
		if (curr.nodeType == Node.TEXT_NODE && curr.parentNode.nodeType == Node.ELEMENT_NODE && typeof curr.parentNode.tagName == "string") {
			try {
				...
				if (tagName == "div" || tagName == "span" || tagName == "p" || tagName == "td") str += curr.nodeValue + "\n";
			} catch (ex) {
				console.dir(curr);
			}
		}
...
PageZipper.prototype.getPosterImagesOnPage = function (page) {
	...
	var isFillerImg = function isFillerImg(img) {
	...
	var getBiggestImg = function getBiggestImg(imgs) {
		var biggestImg = null;
		...
		for (var j = 0; j < pgzp.pages[i].posterImgs.length; j++) {
			...
			if (pos.y > browserBorderTop) {
				if (currPosterImg['pgzpCenterOffset']) {
					...
					if (pos.y > adjustedBrowserBorderTop) {} else {
						continue;
					}
				}
...
PageZipper.prototype.addMenu = function () {
	...
	var assignLinkHandler = function assignLinkHandler(linkId, eventHandler) {
...
PageZipper.prototype.menuSetCurrPageNumber = function (currPage) {
	...
	if (pgzp.displayMode == 'text') {
		...
		var disableNext = currPage == pgzp.pages.length && currPageObj.posterImgs && pgzp.findPos(currPageObj.posterImgs[currPageObj.posterImgs.length - 1]).y < top + pgzp.poster_image_min_vmargin + 1;
...
PageZipper.prototype.goToNext = function (inc) {
	...
	if (pgzp.displayMode == 'text') {
		...
		if (inc > 0) pgzp.goToNextPosterImage();else pgzp.goToPreviousPosterImage();
	}
};
...
PageZipper.prototype.NextLink = function () {
	function _class(text, link, alreadyLoaded, isHumanReadableText) {
	...
	_createClass(_class, [{
		key: "addScore",
		value: function addScore(trialName, score, isNormalized) {
		...
		key: "getScore",
		value: function getScore(trialName, isNormalized) {
...
PageZipper.prototype.Trial = function () {
	function _class2(doScore, weight) {
		var noNormailization = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
...
PageZipper.prototype.trials = {
	'contains_next_syn': new PageZipper.prototype.Trial(function (nextLink) {
		var i,
		    currWord,
		    score = 0;
		for (i = 0; i < pgzp.nextSynonyms.length; i++) {
			...
			if (nextLink.text.toLowerCase().indexOf(currWord.syn) >= 0) {
				if (currWord['humanReadableOnly']) {
					if (!nextLink.isHumanReadableText || nextLink.text.toLowerCase().indexOf("comment") >= 0) continue;
				}
	...
	'url_similarity': new PageZipper.prototype.Trial(function (nextLink) {
	...
	'duplicate_text': new PageZipper.prototype.Trial(function (nextLink) {
		var score = 100;
	...
	'url_ends_in_number': new PageZipper.prototype.Trial(function (nextLink) {
		var results = nextLink.url.match(/^.*?(\d+)(\/+|\.\w+)?$/);
		if (results && parseInt(results[1], 10) < 99) return 100;else return 0;
	}, 20),
	'begins_or_ends_with_next_syn': new PageZipper.prototype.Trial(function (nextLink) {
		if (nextLink.syn && (pgzp.startsWith(nextLink.syn, nextLink.text.toLowerCase()) || pgzp.endsWith(nextLink.syn, nextLink.text.toLowerCase()))) return 100;else return 0;
	}, 20),
	'text_size': new PageZipper.prototype.Trial(function (nextLink) {
		return Math.floor(nextLink.link.offsetWidth * nextLink.link.offsetHeight / nextLink.text.length);
	}, 10),
	'chars_in_text': new PageZipper.prototype.Trial(function (nextLink) {
...
PageZipper.prototype.getNextLink = function (body) {
	...
	if (pgzp.pages.length > 1 && !pgzp.pages[0].nextLinkObj.isSynNumber() && !highestLink.isSynNumber() && pgzp.pages[0].nextLinkObj.syn != highestLink.syn) return null;
	return highestLink;
};
...
PageZipper.prototype.getAllNextLinks = function (body) {
	...
	for (var i = 0; i < links.length; i++) {
		if (links[i].href && pgzp.getDomain(links[i].href) == pgzp.currDomain && (links[i].href.indexOf("#") < 0 || pageUrl != pgzp.getUrlWOutAnchors(links[i].href))) {
...
PageZipper.prototype.addLinkComponents = function (link, allNextLinks, alreadyLoaded) {
	...
	var search = function search(rootNode) {
...
PageZipper.prototype.getCurrentPageNumberFromPageBar = function (allNextLinks) {
	...
	var nextLinkInPagebar = function nextLinkInPagebar(index, pageBar) {
		return index < pageBar.length - 1 ? pageBar[index + 1] : null;
	};
	var pushCurrSeq = function pushCurrSeq() {
		if (currSeq.length > 0) {
	...
	this.loadPage = function (url) {
		pgzp.jq.ajax({
			...
			success: function success(data) {
				...
				data = data.replace(/<script[\w\W]*?>[\w\W]*?<\/script>/ig, '').replace(/<script[\w\W]*?\/>/ig, '').replace(/<noscript>([\w\W]*?)<\/noscript>/ig, "$1").replace(/<meta HTTP-EQUIV=["']?refresh["']?[\w\W]*?>/ig, '');
...
PageZipper.prototype._runOnIframeLoad2 = function () {
	...
	if (ifrDoc.body && ifrDoc.body.childNodes.length > 0 && validReadyStates.test(ifrDoc.readyState)) {
...
PageZipper.prototype.logList = function (list, initialStr, listStr) {
	var interpolate = function interpolate(s, o) {
...
PageZipper.prototype.getContentType = function () {
	...
	for (var i = 0; i < metas.length; i++) {
		if (metas[i].getAttribute('http-equiv') && metas[i].getAttribute('http-equiv').toLowerCase() == 'content-type' && metas[i].getAttribute('content')) return metas[i].getAttribute('content');
	}
	return null;
};

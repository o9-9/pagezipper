...
PageZipper.prototype.loadPageZipper = function () {
  ...
  if (!pgzp.win.Node) {
    pgzp.win.Node = { ELEMENT_NODE: 1, TEXT_NODE: 3 }
  }
...
PageZipper.prototype.getCurrentPage = function () {
  ...
  var currViewBottom =
    pgzp.screen.getScrollTop() + pgzp.screen.getViewportHeight()
  for (i = 0; i < pgzp.pages.length; i++) {
    ...
    if (i == pgzp.pages.length - 1) {
      ...
      if (currPageBottom < currViewBottom) {
...
PageZipper.prototype.getViewableCurrentPage = function (currPage) {
  ...
  if (
    pgzp.findPos(currPageObj.inPageElem).y -
      Math.abs(pgzp.screen.getScrollTop()) >
    pgzp.screen.getViewportHeight() / 2
  ) {
    return currPage - 1
  }
  return currPage
...
PageZipper.prototype.calculateDisplayMode = function (currPage) {
  ...
  txtP.appendChild(
    pgzp.doc.createTextNode(pgzp.getAllTextOnPage(currPage.elemContent))
  )
  ...
  if (currPage.posterImgs == null) {
    currPage.posterImgs = pgzp.getPosterImagesOnPage(currPage.elemContent)
  }
...
PageZipper.prototype.getAllTextOnPage = function (pageHtml) {
  ...
  pgzp.depthFirstRecursion(pageHtml, function (curr) {
    if (
      ...
      curr.parentNode.nodeType == Node.ELEMENT_NODE &&
      typeof curr.parentNode.tagName == 'string'
    ) {
      try {
...
PageZipper.prototype.setPageVisibility = function (currPageIndex) {
  ...
  if (currPageIndex > 5) {
    ...
    if (oldPage.elemContent.style.visibility != 'hidden') {
      oldPage.elemContent.style.visibility = 'hidden'
    }
  }
}
...
PageZipper.prototype.addMenu = function () {
  var css =
    "																																		#pgzp_menu a, #pgzp_menu a * {border: 0; text-decoration: none;}																			#pgzp_menu {position: fixed; top: 0px; right: 8px; padding: 0px 5px; background-color: #D3D3D3; color: black; z-index: 2147483647;}			.pgzp_block {display: block; float: left; line-height: 32px;}																				.pgzp_button {display: block; width: 32px; height: 32px;}																					a.pgzp_button_prev_active {background: transparent url('${media_path}prev.png') no-repeat scroll top left; }						a:hover.pgzp_button_prev_active {background-image: url('${media_path}prev-red.png'); }												a.pgzp_button_prev_inactive {background: transparent url('${media_path}prev-gray.png') no-repeat scroll top left; }				a.pgzp_button_next_active {background: transparent url('${media_path}next.png') no-repeat scroll top left; }						a:hover.pgzp_button_next_active {background-image: url('${media_path}next-red.png'); }												a.pgzp_button_next_inactive {background: transparent url('${media_path}next-gray.png') no-repeat scroll top left; }				#pgzp_button_compat {padding-left: 6px;}																									.pgzp_button_compat_active {background: transparent url('${media_path}compat.png') no-repeat scroll 4px 4px;}							.pgzp_button_compat_inactive {background: transparent url('${media_path}compat-inactive.png') no-repeat scroll 4px 4px;}				#pgzp_curr_page {font-size: 24px;}																											#pgzp_loaded_pages {font-size: 18px;}																									"
  var html =
    "																																									<div id='pgzp_menu'>																																							<a href='#' id='pgzp_button_prev' class='pgzp_block pgzp_button pgzp_button_prev_active' title='Previous - Cntrl Up or Cntrl <'></a>				<a href='#' id='pgzp_button_next' class='pgzp_block pgzp_button pgzp_button_next_active' title='Next - Cntrl Down or Cntrl >'></a>				<div class='pgzp_block' style='padding-left: 5px;'><span id='pgzp_curr_page' title='Current Page'>1</span><span id='pgzp_loaded_pages' title='Pages Loaded'>/1</span></div>											<a href='#' id='pgzp_button_compat' class='pgzp_block pgzp_button pgzp_button_compat_inactive' title='Compatibility Mode - Slower, but less likely to encounter problems'></a>				<a href='https://o9ll.com/pagezipper' target='_blank' title='PageZipper Home' class='pgzp_block pgzp_button' style='margin-left: -6px'>							<img src='${media_path}32.png' alt='PageZipper!' style='border-width: 0px' />													</a>																																									</div>																																									"
...
PageZipper.prototype.NextLink = function (
  text,
  link,
  alreadyLoaded,
  isHumanReadableText
  ...
  this.link = link
  this.syn = ''
  this.isHumanReadableText =
    isHumanReadableText === undefined ? true : isHumanReadableText
...
PageZipper.prototype.trials = {
    ...
    doScore: function (nextLink) {
      ...
      for (i = 0; i < pgzp.nextSynonyms.length; i++) {
        ...
        if (nextLink.text.toLowerCase().indexOf(currWord.syn) >= 0) {
          ...
          if (
            !currWord.humanReadableOnly &&
            nextLink.url.toLowerCase().indexOf(currWord.syn) >= 0
          ) {
  ...
  duplicate_text: {
    doScore: function (nextLink) {
      var score = 100
      if (
        pgzp.linkTextIndex[nextLink.text] &&
        pgzp.linkTextIndex[nextLink.text].length > 0
      ) {
  ...
  begins_or_ends_with_next_syn: {
    doScore: function (nextLink) {
      if (
        nextLink.syn &&
        (pgzp.startsWith(nextLink.syn, nextLink.text.toLowerCase()) ||
          pgzp.endsWith(nextLink.syn, nextLink.text.toLowerCase()))
      ) {
        return 100
      } else {
        return 0
  ...
  text_size: {
    doScore: function (nextLink) {
      return Math.floor(
        (nextLink.link.offsetWidth * nextLink.link.offsetHeight) /
          nextLink.text.length
      )
    },
    weight: 10
  },
...
PageZipper.prototype.getNextLink = function (body) {
  ...
  if (
    pgzp.pages.length > 1 &&
    !pgzp.pages[0].nextLinkObj.isSynNumber() &&
    !highestLink.isSynNumber() &&
  ...
  for (var trial in pgzp.trials) {
    if (pgzp.trials.hasOwnProperty(trial)) {
      for (var i = 0; i < allNextLinks.length; i++) {
        allNextLinks[i].addScore(
          trial,
          pgzp.trials[trial].doScore(allNextLinks[i])
        )
        if (
          trial == 'contains_next_syn' &&
          allNextLinks[i].getScore('contains_next_syn') <= 0
        ) {
...
PageZipper.prototype.normalizeLinks = function (allLinks) {
  for (var trial in pgzp.trials) {
    if (
      pgzp.trials.hasOwnProperty(trial) &&
      !pgzp.trials[trial].noNormailization
    ) {
  ...
  for (var i = 0; i < links.length; i++) {
    if (
      ...
      (links[i].href.indexOf('#') < 0 ||
        pageUrl != pgzp.getUrlWOutAnchors(links[i].href))
    ) {
      pgzp.addLinkComponents(
        links[i],
        allNextLinks,
        pgzp.contains(pgzp.url_list, links[i].href)
      )
    }
  }
  return allNextLinks
}
PageZipper.prototype.addLinkComponents = function (
  link,
  allNextLinks,
  alreadyLoaded
) {
  ...
  var search = function (rootNode) {
    for (var i = 0; i < rootNode.childNodes.length; i++) {
      ...
      if (
        ...
        if (
          ...
          if (curr.src) {
            allNextLinks.push(
              new NextLink(curr.src, link, alreadyLoaded, false)
            )
          }
        } else {
          search(curr)
    ...
    if (rootNode.title) {
      allNextLinks.push(new NextLink(rootNode.title, link))
    }
    if (rootNode.alt) {
      allNextLinks.push(new NextLink(rootNode.alt, link))
    }
  }
  search(link)
}
...
function PageZipperAjax() {
  this.loadPage = function (url) {
    pgzp.jq.ajax({
      ...
      success: function (data) {
        ...
        data = data
          .replace(/<script[\w\W]*?>[\w\W]*?<\/script>/gi, '')
          .replace(/<script[\w\W]*?\/>/gi, '')
          .replace(/<noscript>([\w\W]*?)<\/noscript>/gi, '$1')
          .replace(/<meta HTTP-EQUIV=["']?refresh["']?[\w\W]*?>/gi, '')
  ...
  this.removeAbsolutePositioning = function (body) {
    pgzp
      ...
      .children()
      .each(function () {
        if (pgzp.jq(this).css('position') == 'absolute') {
          pgzp.jq(this).css('position', 'static')
        }
      })
  }
}
...
function PageZipperIframe() {
  this.loadPage = function (url) {
    ...
    var onloadstr =
      pgzp.loader_type == 'ffextension'
        ? ''
        : 'onload="pgzp.iframe.processPageAddComplete(\'pgzp_page' +
          pgzp.pages.length +
          '\')"'
    ifrspan.innerHTML =
      "<iframe id='pgzp_page" +
      pgzp.pages.length +
      "' class='pgzp_page_added' " +
      onloadstr +
      " style='clear: both; display: block; border-width: 0; overflow: hidden;' scroll='no' frameBorder='0'></iframe>"
  ...
  this.processPageAddComplete = function (ifrId) {
    ...
    if (page) {
      pgzp.iframe.setIFrameHeight(page)
    }
  }
